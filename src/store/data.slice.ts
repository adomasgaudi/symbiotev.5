...
const dataSlice = createSlice({
  ...
  reducers: {
    ...
    editThis: (
      state,
      action: P.a<[keyof Data.storeDataType, Data.stateObj]>
    ) => {
    ...
    updateSym: (state, action: P.a<Data.symType>) => {
      ...
      if (state.pageDoc)
        state.pageDoc.syms?.forEach((sym: Data.symType) => {
          if (sym.symId === action.payload.symId) sym.body = action.payload.body
        })
    },
    ...
    updateTitle: (
      state,
      action: P.a<{ title: string; order: number; docId: string }>
    ) => {
      if (state.userDocs)
...
export const { editThis, updateSym, updateTitle } = dataSlice.actions
